...
const registerUser = asyncHandler(async (req,res)=>{
    ...
    if(existedUser){
        throw new apiError (409,"A user already registerd with same username and email")
    }
    ...
    if (req.files && Array.isArray(req.files.coverImage) && req.files.console.length>0) {
...
const updateAccountDetails = asyncHandler(async(req,res)=>{
    ...
    return res
    .status(200)
    .json(new apiResponse(400,user,"Account details updates successfully"))
})
...
const updateUserCoverImage= asyncHandler(async(req,res)=>{
    ...
    if(!(coverImage.url)){
        throw new apiError(400,"Error while uploading on cloud(for)coverimage")
    }
...
const getWatchHistory = asyncHandler(async(req,res)=>{
    ...
    .json(
        200,
        user[0].watchHistory,
        "Watched history fectched successfully"
    )
})
export {registerUser,
    loginUser,
